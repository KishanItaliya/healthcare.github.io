<?php
   
   $conn = mysqli_connect("localhost","root","","hms");
   if(!$conn){
       die("ERROR:".mysqli_connect_error());
   }

   extract($_POST);
   
   if(isset($_POST["readrecord"])){
    $data = '<table class="table table-bordered text-center table-striped table-fixed">
                <tr>
                    <th>Patient ID</th>
                    <th>Name</th>
                    <th>Mobile</th>
                    <th>Email</th>
                    <th>Date of Birth</th>
                    <th>Edit</th>
                    <th>Create</th>
                    <th>Delete</th>
                </tr>';

    $sql = "SELECT * FROM doctor";
    $result = mysqli_query($conn,$sql);
    
    if(mysqli_num_rows($result) > 0){

        while($row = mysqli_fetch_assoc($result)){    
            
            $data .= '<tr>
                        <td>'.$row['d_id'].'</td>
                        <td>'.$row["d_name"].'</td>
                        <td>'.$row["d_mobile"].'</td>
                        <td>'.$row["d_email"].'</td>  
                        <td>'.$row["d_dob"].'</td>   
                        
                        <td><button onclick="GetDoctor('.$row['d_id'].')" 
                             class="btn btn-warning">Edit</button>
                        </td>

                        <td><button onclick="GetUser('.$row['d_id'].')" 
                             class="btn btn-success">LoginID</button>
                        </td>

                        <td>
                             <button onclick="DeleteDoctor('.$row['d_id'].')" 
                             class="btn btn-danger">Delete</button>
                        </td>

                      </tr>';
        }
    }
    $data.= '</table>';
    echo $data;
}

    //---------- Add User ------------

    if(isset($_POST["name"]) && isset($_POST["email"]) && isset($_POST["gender"]) && isset($_POST["dob"])
        && isset($_POST["status"]) && isset($_POST["mobile"]) && isset($_POST["address"]) 
        && isset($_POST["uname"]) && isset($_POST["pwd"]) && isset($_POST["state"])){
        
        //$file = addslashes(file_get_contents($_FILES["profile"]["tmp"]));  
 
        $doctor = "Doctor";

        $sql = "INSERT INTO `doctor`(`d_name`, `d_gender`, `d_mobile`, `d_email`, `d_dob`, `d_status`, `d_state`, `d_address`)
                 VALUES ('$name','$gender','$mobile','$email','$dob','$status','$state','$address')";
        mysqli_query($conn,$sql);

        $usql = "INSERT INTO `users`(`u_type`,`u_name`, `u_username`, `u_password`, `u_mobile`, `u_email`, `u_dob`, `u_address`, `u_state`) 
                VALUES ('$doctor','$name','$uname','$pwd','$mobile','$email','$dob','$address','$state')";
        mysqli_query($conn,$usql);
           
    }

   //---------- Delete Doctor -------------

   if(isset($_POST["deleteid"])){
        $did = $_POST["deleteid"];
        $sql = "DELETE FROM doctor WHERE d_id='$did'";
        mysqli_query($conn,$sql);
    }

    //--------- Get DoctorID For Update -----------

    if(isset($_POST['id']) && isset($_POST['id']) !=""){

        $did = $_POST['id'];
        $sql = "SELECT * FROM doctor WHERE d_id = '$did'";
        
        if(!$result = mysqli_query($conn,$sql)){
            exit(mysqli_error());
        }

        $response = array();

        if(mysqli_num_rows($result) > 0){
            while($row = mysqli_fetch_assoc($result)){

                $response = $row;
            }
        }
        else{
            $response['status'] = 200;
            $response['message'] = "Data not found";
        }
        echo json_encode($response);
    }

    else{
        $response['status'] = 200;
        $response['message'] = "Invalid Request!";
    }


    //--------- Update Doctor ---------------

    if(isset($_POST['hidden_user_idupd'])){

        $hidden_user_idupd = $_POST['hidden_user_idupd'];
        $nameupd = $_POST['nameupd'];
        $emailupd = $_POST['emailupd'];
        $genderupd = $_POST['genderupd'];
        $dobupd = $_POST['dobupd'];
        $statusupd = $_POST['statusupd'];
        $mobileupd = $_POST['mobileupd'];
        $addressupd = $_POST['addressupd'];
        $stateupd = $_POST['stateupd'];

        $sql = "UPDATE `doctor` SET `d_name`='$nameupd',`d_email`='$emailupd',`d_gender`='$genderupd',`d_dob`='$dobupd',`d_status`='$statusupd',
                `d_mobile`='$mobileupd',`d_address`='$addressupd',`d_state`='$stateupd' WHERE d_id='$hidden_user_idupd'";
        mysqli_query($conn,$sql);
    }


    //--------- Get UserID For Update -----------

    if(isset($_POST['id1']) && isset($_POST['id1']) !=""){

        $did = $_POST['id1'];

        $dsql = "SELECT d_name FROM doctor WHERE d_id='$did'";
        $result = mysqli_query($conn,$dsql);
        $row = mysqli_fetch_assoc($result);
        $name = $row["d_name"];

        $sql = "SELECT * FROM users WHERE u_name = '$name'";
        
        if(!$result = mysqli_query($conn,$sql)){
            exit(mysqli_error());
        }

        $response = array();

        if(mysqli_num_rows($result) > 0){
            while($row = mysqli_fetch_assoc($result)){
                $response = $row;
            }
        }
        else{
            $response['status'] = 200;
            $response['message'] = "Data not found";
        }
        echo json_encode($response);
    }

    else{
        $response['status'] = 200;
        $response['message'] = "Invalid Request!";
    }


    //--------- Update User ---------------

    if(isset($_POST['hidden_user_idupd1'])){

        $hidden_user_idupd1 = $_POST['hidden_user_idupd1'];
        $unameupd = $_POST['unameupd'];
        $pwdupd = $_POST['pwdupd'];

        $psql = "SELECT * FROM doctor WHERE d_id='$hidden_user_idupd1'";
        $result = mysqli_query($conn,$psql);
        $row = mysqli_fetch_assoc($result);
        $name = $row["d_name"];

        $sql = "SELECT u_name FROM `users` WHERE u_name='$name'";
        $result = mysqli_query($conn,$sql);
        $row1 = mysqli_fetch_assoc($result);
        $user = $row1["u_name"];

        if($user!=''){
            $sql = "UPDATE `users` SET `u_username`='$unameupd',`u_password`='$pwdupd' WHERE u_name='$name'";
            mysqli_query($conn,$sql);
        }

        else{

            $u_type = "Doctor";
            $mobile = $row["d_contact"];
            $email = $row["d_email"];
            $dob = $row["d_dob"];
            $address = $row["d_address"];
            $state = $row["d_state"];
            
            $usql = "INSERT INTO `users`(`u_type`,`u_name`, `u_username`, `u_password`, `u_mobile`, `u_email`, `u_dob`, `u_address`, `u_state`) 
                     VALUES ('$u_type','$name','$unameupd','$pwdupd','$mobile','$email','$dob','$address','$state')";
            mysqli_query($conn,$usql);
        }
        
    }


?>